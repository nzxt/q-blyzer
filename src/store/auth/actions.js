...
const REFRESH_TOKEN_ROUTE = '/api/account/refreshTokens'
...
export function login (state, data) {
  const p = new Promise(function (resolve, reject) {
    return axiosInstance.post(LOGIN_ROUTE, data.body).then((response) => {
      ...
      state.dispatch('setToken', { token: token, rememberMe: data.rememberMe, tokens: response.data })
...
export function setToken (state, data) {
  ...
  if (data.rememberMe) {
    Cookies.set('tokens', data.tokens, {
      expires: 365
    })
